// datagram/subscriber — receives QUIC RFC 9221 unreliable datagrams and reports
// per-second delivery statistics including sequence gaps (visible packet loss).
//
// Unlike stream-based PUB/SUB, datagram delivery is not retransmitted by QUIC.
// Packet loss appears directly as seq_gaps in the JSON result, making this
// ideal for thesis comparison with stream mode under the same loss conditions.
//
// Expected message format: <topic>|<seq>|<send_ns>|<padding>
//
// Usage:
//
//	go run ./example/datagram/subscriber -id sub1 -dur 30s -output sub1.json
//
// Flags:
//
//	-addr         quic://127.0.0.1:9300   publisher address
//	-topic        sensor                   subscription prefix ("" = all)
//	-dur          30s                      receive duration
//	-skip-warmup  3s                       discard early samples (post-handshake baseline)
...
func main() {
	addr       := flag.String("addr",        "quic://127.0.0.1:9300", "publisher address")
	topic      := flag.String("topic",      "sensor",                "subscription prefix")
	...
	log.Printf("[%s] datagram subscriber → %s  topic=%q  dur=%s", *id, *addr, *topic, *dur)
...
loop:
	...
	printWindow(*id+" FINAL", totalRcvd, gaps, allLats)
...
func printWindow(id string, total, gaps int64, lats []float64) {
	...
	fmt.Printf("[%s] rcvd=%-6d gaps=%-4d p50=%.2fms p99=%.2fms max=%.2fms\n",
		id, total, gaps,
